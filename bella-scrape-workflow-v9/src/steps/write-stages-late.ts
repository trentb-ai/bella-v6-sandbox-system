// Step 19s: Write script_stages (late duplicate) — lead:{lid}:script_stages TTL 86400
// NOTE: This step does NOT rethrow errors (intentional — matches deployed.js)
import type { Env, WorkflowResults, WorkflowState, StepFn, WorkflowPayload } from '../lib/types';

export async function writeStagesLate(
  step: StepFn,
  env: Env,
  ...
  try {
    console.log("type:WF_NODE_START:nodeId:node-kv-write-stages:nodeName:kv-put:nodeType:kv-put:timestamp:" + Date.now() + ":instanceId:" + instanceId);
    results.step_kv_put_script_stages = await step.do("step_kv_put_script_stages", async () => {
      ...
      state["node-kv-write-stages"] = { input: {}, output: result };
      return result;
    });
    state["node-kv-write-stages"] = state["node-kv-write-stages"] || { output: results.step_kv_put_script_stages };
    console.log("type:WF_NODE_END:nodeId:node-kv-write-stages:nodeName:kv-put:nodeType:kv-put:timestamp:" + Date.now() + ":instanceId:" + instanceId + ":success:true:output:" + JSON.stringify(state["node-kv-write-stages"]?.output));
  } catch (error) {
    var errorMessage = error instanceof Error ? error.message : String(error);
    console.log("type:WF_NODE_ERROR:nodeId:node-kv-write-stages:nodeName:kv-put:nodeType:kv-put:timestamp:" + Date.now() + ":instanceId:" + instanceId + ":success:false:error:" + errorMessage);
