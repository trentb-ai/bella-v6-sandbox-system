...
3. **Logic Gate Mismatch:** The code assumes a `body._v9_leadId` exists (line **6061**) to trigger the V3 merge, but doesn't have a fallback "Minimum Viable KV Write" if the background task is still "Processing".
