/**
 * bella-voice-agent v9.0.0
 * Architecture: Browser WebSocket → Durable Object → Deepgram Voice Agent WS
 * v9.0.0: Director opening with SC ranking, 5-agent ROI, parallel KV intel (layered on MCP)
 */
...
const log = (tag: string, msg: string, t0?: number) => {
  ...
  console.log(`[BellaV5 ${VERSION}] [${tag}]${elapsed} ${msg}`);
};

// ── Deepgram config ──────────────────────────────────────────────────────────
const DG_VOICE = "aura-2-theia-en";
const DG_STT_MODEL = "nova-3";
const DG_LLM_MODEL = "gemini-2.0-flash"; // used as model label only — actual inference via bridge
const DG_LLM_PROVIDER = "custom"; // V5: BYO LLM bridge
const DG_WS_URL = "wss://agent.deepgram.com/v9/agent/converse";
const KEEPALIVE_MS = 5000; // Deepgram drops idle connections without this

// ── v9.0.0: ROI agent constants ──────────────────────────────────────────────
...
interface Env {
  ...
  TOOLS: Fetcher;              // V5: bella-tools-worker-v9
  CONSULTANT: Fetcher;         // V5: consultant-sandbox-v9 (freeform Tier 2)
  DEEPGRAM_API_KEY: string;
  MCP: Fetcher;                // leads-mcp-worker-sandbox-v9 (service binding)
  BRIDGE_URL: string;           // Deepgram calls this externally — must be public HTTPS (not a service binding)
  TOOLS_BEARER: string;        // V5: bearer token for tools worker
  GHL_LOCATION_ID: string;
}

// v9.0.0: expanded state for SC ranking + ROI data
...
export class BellaAgent extends Agent<Env, BellaState> {
  ...
  async onStart() {
    if (!this.state?.history) {
      this.setState({ openingFired: false, history: [], scRank: [], scFlags: {}, intelLoaded: false });
    }
  }
  ...
  // ── Intel fetch + system prompt build (v5.0.0: 3-layer approach) ───────────
  ...
  private async loadIntelAndConnect(connection: any, t0: number, retries = 0) {
    ...
    try {
      const res = await this.env.MCP.fetch(`https://mcp-internal/resolve-intel`, {
        method: "POST",
    ...
    try {
      const [intelStr, statusStr] = await Promise.all([
        this.env.LEADS_KV.get(`lead:${this.lid}:intel`),
        this.env.LEADS_KV.get(`lead:${this.lid}:status`),
      ]);
      log("INTEL", `KV pipeline: intel=${!!intelStr} status="${statusStr}"`, t0);
    ...
    // ── v9.0.0: If KV pipeline intel exists, use structured SC prompt ────────
    if (kvIntel) {
      ...
      // ROI not pre-calculated — Bella collects ACV from prospect then calls get_roi_confirmed
      ...
      this.systemPrompt = basePrompt + `\n\n--- PROSPECT INTEL (background knowledge — do NOT read aloud) ---\n${intelLines.join("\n")}\n--- END INTEL ---\nCRITICAL: All scraped intel above is BACKGROUND KNOWLEDGE only. Weave it naturally into conversation.\nDo NOT quote pre-calculated ROI dollar amounts — those are not confirmed. Collect ACV from the prospect first.\nWhen the prospect shares their ACV, IMMEDIATELY call capture_acv. Then call get_roi_confirmed for real calculated figures.\nYour lead ID for tool calls is: ${this.lid}`;
...
The opening greeting has just been spoken. Begin Stage 1: ask their role, then what brings them here today.`;
    ...
    // ── Null-guard: if no data at all, don't open WS ─────────────────────────
    if (!intelSuccess && !kvIntel && !intel.business_name) {
      log("INTEL", `null-guard: intelSuccess=${intelSuccess} kvIntel=${!!kvIntel} biz="${intel.business_name}" — skipping WS open`, t0);
      this.sendJSON(connection, {
        type: "pipeline_pending",
        message: "We're still pulling your business data — give it a moment and refresh to try again.",
      });
      return;
    }

    // ── V3: raw_only retry — no pipeline retrigger, just wait for scraper ctx.waitUntil ────
    if (intel.pipeline_status === "raw_only" && !kvIntel && retries < 1) {
      log("INTEL", `raw_only detected (retry ${retries}) — waiting for scraper background tasks`, t0);
      this.sendJSON(connection, {
        type: "pipeline_pending",
        message: "Finishing your business analysis — reconnecting in a few seconds...",
      });
      await new Promise(r => setTimeout(r, 4000));
      return this.loadIntelAndConnect(connection, t0, retries + 1);
    }
    ...
    // ── v9.0.0: Director opening when SC + ROI data available ────────────────
    let openingText: string;
    if (false) {
      // Legacy ROI director opening removed — Bella follows prescribed 22-stage script
    } else {
  ...
  // ── v9.0.0: Structured system prompt from SC intel ─────────────────────────
  private buildSystemPromptV3(intel: any, businessName: string, basePrompt: string): string {
    if (!intel) return basePrompt + "\n\nNO INTEL LOADED. Introduce yourself warmly and ask their name and business.";
...
(Background knowledge — weave naturally, never read aloud as a list)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
CHANNEL FLAGS (use to know which questions to prioritise):
...
Database Reactivation Opportunity: ${flags.database_reactivation ? "YES" : "NO"}

AGENT PRIORITY ORDER (routing only — no dollar figures yet):
${rank.join(" → ")}
Priority agents: ${(routing.priority_agents ?? []).join(", ")}
Skip agents: ${(routing.skip_agents ?? []).join(", ")}
${Object.entries(routing.reasoning ?? {}).map(([a, reason]) => `  ${a}: ${reason}`).join("\n")}

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
CONSULTANT SCRIPT FILLS (use verbatim where marked)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
REPUTATION COMMENTARY: ${sf.rep_commentary ?? ""}
BEST REVIEW SNIPPET: ${sf.recent_review_snippet ?? ""}
REP QUALITY ASSESSMENT: ${sf.rep_quality_assessment ?? ""}
TOP WEBSITE CTAs: ${sf.top_2_website_ctas ?? ""}

LANDING PAGE VERDICT: ${lp.verdictLine ?? ""}
Conversion Barriers: ${(lp.conversionBarriers ?? []).join("; ")}
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
VOICE CALL RULES:
- No markdown. No bullet points. 2-3 sentences max per turn. Speak naturally.
- Follow the 22-stage script in brain:bella:script_kb exactly.
- NEVER quote pre-calculated ROI dollar amounts. You do not have confirmed figures yet.
- ACV comes from the prospect — ask for it at Stage 8. Accept any ballpark.
- When the prospect gives ACV, IMMEDIATELY call capture_acv tool.
- On your next turn after capture_acv, call get_roi_confirmed for real calculated figures.
- Only quote ROI dollar amounts AFTER get_roi_confirmed returns confirmed data.
- Scraped intel above is background knowledge — weave it in naturally, do not list it.
- Your lead ID for tool calls is: ${this.lid}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`;
  }
    ...
    dgWs.addEventListener("open", () => {
      ...
      const settings: Record<string, any> = {
        ...
        agent: {
          ...
          think: {
            // V5: BYO LLM bridge — Gemini Flash 2.0 via deepgram-bridge-v9
            provider: {
              type: "custom",
              url:  "https://deepgram-bridge-sandbox-v9.trentbelasco.workers.dev/v9/chat/completions",
            },
            prompt: this.systemPrompt,
            // V5: Tools registered with Deepgram for mid-call tool calls
            // Bridge also exposes Tier1/Tier2 tools to Gemini directly
            functions: TOOLS,
          },
          speak: {
            provider: {
  ...
  private async handleToolCall(msg: Record<string, any>, dgWs: WebSocket, t0: number) {
    ...
    try {
      // V5: TOOLS service binding — always available, no URL fallback needed
      let res: Response;
      if (this.env.TOOLS) {
        res = await this.env.TOOLS.fetch(`https://tools-internal/${fnName}`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            "Authorization": `Bearer ${(this.env as any).TOOLS_BEARER ?? "bella-tools-secret"}`,
          },
          body: JSON.stringify(args),
        });
      } else if (false) {
        // V5: no URL fallback — TOOLS binding is required
        res = new Response(JSON.stringify({ error: "TOOLS binding not configured" }), { status: 500 });
        res = await fetch(`https://unused/${fnName}`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            "Authorization": `Bearer ${(this.env as any).TOOLS_BEARER ?? "bella-tools-secret"}`,
        ...
        throw new Error("TOOLS service binding is required — check wrangler.toml");
      }
