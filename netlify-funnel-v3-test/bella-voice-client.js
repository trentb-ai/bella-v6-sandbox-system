  ...
  window.BellaVoiceClient = {
    async init() {
      try {
        ...
        micStarted = true;
        console.log('[BellaV1] Microphone started');
        return true;
      } catch (e) {
        console.error('[BellaV1] Mic error:', e);
        return false;
      }
    },
    ...
    connect(onMessage, onError) {
      ...
      console.log('[BellaV1] Connecting to:', url);
      
      ws = new WebSocket(url);
      ws.binaryType = 'arraybuffer';
      
      ws.onopen = () => {
        console.log('[BellaV1] Connected');
        this.init();
      };
      ...
      ws.onerror = (evt) => {
        console.error('[BellaV1] WS error', evt);
        if (onError) onError(evt);
      };
      
      ws.onclose = () => {
        console.log('[BellaV1] Disconnected');
      };
    },
