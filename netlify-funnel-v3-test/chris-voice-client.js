...
(function () {
  ...
  async function startCall() {
    ...
    ws.onopen = () => {
      ...
      connected = true;
      binaryMsgCount = 0;
      window._chrisWS = ws;  // V3 Fix A: expose WS for inject_context shim
    ...
    ws.onmessage = async (evt) => {
      if (evt.data instanceof ArrayBuffer) {
        binaryMsgCount++;
        if (ignoreAudio) {
          console.log(`[ChrisV1] DROPPED audio #${binaryMsgCount} — post-barge-in`);
          return;
        }
        console.log(`[ChrisV1] audio #${binaryMsgCount} — ${evt.data.byteLength}b`);
        playAudio(evt.data);
        return;
      }
      ...
      console.log('[ChrisV1] ctrl:', msg.type);
      ...
      switch (msg.type) {
        ...
        case 'user_started_speaking':
          ...
          console.log('[ChrisV1] server barge-in: UserStartedSpeaking');
          stopAllAudio();
          setLabel('Chris listening…', 'Tap to end call');
          break;
        case 'listening':
          ...
          break;
        case 'transcript':
          console.log('[ChrisV1 heard]', msg.text);
          break;
        case 'pipeline_pending':
          // Intel still loading — close this WS and auto-retry in 4s
          console.log('[ChrisV1] pipeline_pending — retrying in 4s');
          ...
          break;
        case 'error':
          console.error('[ChrisV1 error]', msg.message);
          break;
      }
    };

    ws.onerror = (e) => {
      console.error('[ChrisV1] WS error', e);
      connecting = false;
  ...
  function cleanup() {
    window._chrisWS = null;  // V3 Fix A: clear global WS ref on disconnect
    connecting = false;
    connected = false;
    bellaActive = false;
    ignoreAudio = false;
  ...
  function startMic() {
    ...
    console.log('[ChrisV1] Mic started at', TARGET_RATE, 'Hz');
  }
  ...
  function getPlaybackCtx() {
    ...
    if (playbackCtx.state === 'suspended') {
      ...
      playbackCtx.resume().catch(e => console.warn('[ChrisV1] playbackCtx resume failed:', e));
    }
    return playbackCtx;
  }
  ...
  function finishBellaUtterance() {
    ...
    bellaActive = false;
    agentAudioDone = false;
    console.log('[ChrisV1] speech done — Chris now listening');
    if (connected) setLabel('Chris listening…', 'Tap to end call');
  }
  ...
  function playAudio(arrayBuffer) {
    ...
    if (scheduledSources.size === 0) {
      console.log(`[ChrisV1] playAudio first chunk — ctx.state=${ctx.state} ctx.sampleRate=${ctx.sampleRate} buf.duration=${decoded.duration.toFixed(3)}s`);
    }
