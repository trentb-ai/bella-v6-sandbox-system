...
| Worker | URL | Version |
|---|---|---|
| Voice Agent DO | bella-voice-agent-sandbox-v5.trentbelasco.workers.dev | v2.9.0 |
...
**Netlify folder to drag-deploy:** /Users/trentbelasco/Desktop/BELLA_V5_SANDBOX_COMPLETE_SYSTEM/netlify-funnel-sandbox
...
```
[V3] ws-shim-captured: wss://bella-voice-agent-sandbox-v5...
...
**The proper fix:**
In `voice-agent-v5-fresh/src/index.ts`, change the null guard so it ALWAYS proceeds to open Deepgram — using the fallback prompt if no intel:
...
1. Tail bridge logs during a call: `cd voice-agent-v5-fresh && npx wrangler tail deepgram-bridge-sandbox-v5 --format pretty`
...
│   ├── loading-v15.html               ← MIN_DURATION reduced 20s→8s (needs deploy)
│   ├── demo_v15_hybrid.html           ← main demo page
│   └── assets/
├── voice-agent-v5-fresh/              ← ACTIVE voice agent source
│   └── src/index.ts                   ← null guard on line ~493, fallback on ~466
├── deepgram-bridge-v5/
│   └── src/index.ts                   ← bridge v5.3.1, extraction fire-and-forget
└── ...other workers...
```
...
| Fix | File | Status |
|---|---|---|
| Bridge extraction: await→fire-and-forget | deepgram-bridge-v5/src/index.ts | ✅ Deployed CF |
| Voice agent null-guard: added urlHints.biz check | voice-agent-v5-fresh/src/index.ts | ✅ Deployed CF |
...
| Loading page: 20s → 8s | netlify-funnel-sandbox/loading-v15.html | ⚠️ Needs Netlify deploy |
...
curl https://leads-mcp-worker-sandbox-v5.trentbelasco.workers.dev/health
...
# Tail logs (run from voice-agent-v5-fresh dir)
cd /Users/trentbelasco/Desktop/BELLA_V5_SANDBOX_COMPLETE_SYSTEM/voice-agent-v5-fresh
