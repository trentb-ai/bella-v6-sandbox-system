...
app.use("*", async (c, next) => {
  if (!envChecked) {
    ...
    try {
      ensureEnv(c.env, requiredEnv as any);
...
app.use("/api/*", async (c, next) => {
  if (c.req.path.startsWith("/api/setup") || c.req.path.startsWith("/api/catalog")) return next();
  return credentialsMiddleware(c as any, async () => {
    return cloudflareMiddleware(c as any, next);
  });
});
