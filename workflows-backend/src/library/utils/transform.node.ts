...
function resolveTemplateExpression(
  ...
  return expr.replace(TEMPLATE_PATTERNS.TEMPLATE_REGEX, (_match, innerExpr) => {
    ...
    const [nodeIdInGraph, ...restId] = trimmed.split(TEMPLATE_PATTERNS.PATH_SEPARATOR);
    const tailId = restId.length ? "." + restId.join(".") : ".output";
    return `_workflowState['${nodeIdInGraph}']${tailId}`;
  });
}
...
export const TransformNode: WorkflowNodeDefinition<TransformConfig> = {
  ...
  codegen: ({
    ...
    return Effect.gen(function*(_) {
      ...
      // No more flattening with replace(/\n/g, " ")!
      // But we need to check for return smartly.
    ...
    _workflowResults.${stepName} = await step.do('${stepName}', async () => {
      ...
      const result = await (async () => { ${codeBody} })();
