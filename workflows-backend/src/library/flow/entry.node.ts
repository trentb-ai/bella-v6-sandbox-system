...
export const EntryNode: WorkflowNodeDefinition<EntryConfig> = {
  ...
  codegen: ({ nodeId, config, stepName }): Effect.Effect<CodeGenResult, { _tag: ErrorCode; message: string }> => {
    return Effect.gen(function* (_) {
      const params = config.params || [];
      const paramNames = params.map(p => p.name).filter(Boolean);
      
      const sanitizedStepName = stepName?.replace(/[^a-zA-Z0-9_]/g, "_") || `step_${nodeId.replace(/[^a-zA-Z0-9]/g, "_")}`;
    ...
    _workflowResults.${sanitizedStepName} = { ${paramList} };
    _workflowState['${nodeId}'] = {
      input: event.payload,
      output: _workflowResults.${sanitizedStepName}
    };`;
      } else {
        code = `
    _workflowResults.${sanitizedStepName} = event.payload || {};
    _workflowState['${nodeId}'] = {
      input: event.payload,
      output: _workflowResults.${sanitizedStepName}
    };`;
