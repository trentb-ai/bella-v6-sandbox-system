...
export const ReturnNode: WorkflowNodeDefinition<ReturnConfig> = {
  ...
  codegen: ({ nodeId, config, stepName, graphContext }): Effect.Effect<CodeGenResult, { _tag: ErrorCode; message: string }> => {
    return Effect.gen(function* (_) {
      ...
      let resultValue: string = inputData;
      
      if (config.value) {
        if (typeof config.value === "string" && config.value.includes("{{")) {
          // Wrap in template literal and JSON.parse if it looks like a JSON string with templates
          const resolved = config.value.replace(TEMPLATE_PATTERNS.TEMPLATE_REGEX, (_match, innerExpr) => {
            return `\${${resolveTemplateExpression(`{{${innerExpr}}}`, graphContext)}}`;
          });
          resultValue = `JSON.parse(\`${resolved}\`)`;
        } else {
          resultValue = JSON.stringify(config.value);
        }
      } else if (config.returnValue?.type === "expression") {
        resultValue = config.returnValue.value;
      } else if (config.returnValue?.value) {
        const returnVal = config.returnValue.value;
        if (typeof returnVal === "object" && returnVal !== null) {
          const json = JSON.stringify(returnVal);
          resultValue = json.replace(/"\{\{([^}]+)\}\}"/g, (_match, innerExpr) => {
            return resolveTemplateExpression(`{{${innerExpr}}}`, graphContext);
          });
        } else if (typeof returnVal === "string" && returnVal.includes("{{")) {
          resultValue = resolveTemplateExpression(returnVal, graphContext);
        } else {
          resultValue = JSON.stringify(returnVal);
        }
      }
