...
export class WorkflowCompiler {
  static compile(
    ...
    return Effect.gen(function* (_) {
      ...
      const tsCode = yield* _(
        WorkflowCompiler.generateWorkerCode(workflow, codegenResults, options, workflowId, graphContext)
      );
  ...
  > {
    return Effect.gen(function* (_) {
      ...
      for (let i = 0; i < graphContext.topoOrder.length; i++) {
        ...
        const stepName = graphContext.stepNameMap.get(nodeId) || nodeId.replace(/[^a-z0-9]/g, "_");
  ...
  private static getNodeDefinition(nodeType: string): WorkflowNodeDefinition<unknown> | null {
    const nodeTypeMap: Record<string, WorkflowNodeDefinition<unknown>> = {
      ...
      [NodeType.PARALLEL]: NodeLibrary.ParallelNode as WorkflowNodeDefinition<unknown>,
      [NodeType.JOIN]: NodeLibrary.JoinNode as WorkflowNodeDefinition<unknown>,
  ...
  private static generateWorkerCode(
    ...
    return Effect.gen(function* (_) {
      ...
      // Walk each PARALLEL node's branch edges to find independent branch chains.
      // These chains will be wrapped in Promise.all() for concurrent execution.
      const consumedByParallel = new Set<string>();
      const parallelBranchMap = new Map<string, Array<{ branchName: string; nodeIds: string[] }>>();

      codegenResults.forEach(({ nodeId, nodeType: nt }) => {
        if (nt !== 'parallel' || !graphContext) return;
        const branchEdges = graphContext.edges.filter(
          e => e.source === nodeId && e.sourceHandle?.startsWith('branch')
        );
        const branches: Array<{ branchName: string; nodeIds: string[] }> = [];

        branchEdges.forEach(edge => {
          const chain: string[] = [];
          let current: string | null = edge.target;
          while (current) {
            chain.push(current);
            consumedByParallel.add(current);
            const outEdges = graphContext.edges.filter(e => e.source === current);
            if (outEdges.length === 1) {
              const nextTarget = outEdges[0].target;
              const nextInEdges = graphContext.edges.filter(e => e.target === nextTarget);
              const allFromBranch = nextInEdges.every(
                e => chain.includes(e.source) || e.source === nodeId
              );
              current = allFromBranch ? nextTarget : null;
            } else {
              current = null;
            }
          }
          branches.push({ branchName: edge.sourceHandle || 'branch1', nodeIds: chain });
        });

        if (branches.length > 0) {
          parallelBranchMap.set(nodeId, branches);
          logger.info("Parallel branches detected", {
            parallelNodeId: nodeId,
            branches: branches.map(b => ({ name: b.branchName, nodes: b.nodeIds }))
          });
        }
      });

      // Helper: generate wrapped code block for a single node
      const wrapNodeCode = (entry: typeof codegenResults[number]) => {
        const { nodeId: nId, nodeLabel: nLabel, nodeType: nType, result: nResult } = entry;
        const nName = nLabel || nType;
        const indented = indentCode(nResult.code, 12);
        return `
        try {
          console.log('type:WF_NODE_START:nodeId:${nId}:nodeName:'+${JSON.stringify(nName)}+':nodeType:'+'${nType}'+':timestamp:'+Date.now()+':instanceId:'+event.instanceId);
${indented}
          console.log('type:WF_NODE_END:nodeId:${nId}:nodeName:'+${JSON.stringify(nName)}+':nodeType:'+'${nType}'+':timestamp:'+Date.now()+':instanceId:'+event.instanceId+':success:true:output:'+JSON.stringify(_workflowState['${nId}']?.output));
        } catch (error) {
          const errorMessage = error instanceof Error ? error.message : String(error);
          console.log('type:WF_NODE_ERROR:nodeId:${nId}:nodeName:'+${JSON.stringify(nName)}+':nodeType:'+'${nType}'+':timestamp:'+Date.now()+':instanceId:'+event.instanceId+':success:false:error:'+errorMessage);
          throw error;
        }`;
      };

      const nodeCodes = codegenResults.map(({ nodeId, nodeLabel, nodeType, result, branchCondition, branchReason }, index) => {
        // Skip nodes consumed by a parallel block — they're already inside Promise.all
        if (consumedByParallel.has(nodeId)) return '';
        ...
        // Extract step result key for replay-safe _workflowState reconstruction.
        // CF Workflows replays run() from the top after waitForEvent/restart — step.do()
        // callbacks don't re-execute, so _workflowState writes inside them are lost.
        // This guard repopulates _workflowState from the cached _workflowResults.
        const stepKeyMatch = nodeCode.match(/_workflowResults\.(\w+)\s*=/);
        const stepKey = stepKeyMatch ? stepKeyMatch[1] : null;
        const replayGuard = stepKey
          ? `\n      _workflowState['${nodeId}'] = _workflowState['${nodeId}'] || { output: _workflowResults.${stepKey} };`
          : '';
