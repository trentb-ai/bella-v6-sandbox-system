...
// ─── Consultant: call consultant-v6 with Firecrawl data ──────────────────────
...
function buildConsultantPayload(
  ...
  const industryGuess =
    ...
    rawText.includes("real estate") || rawText.includes("property") ? "real estate" :
    rawText.includes("plumb") || rawText.includes("electric") || rawText.includes("hvac") ? "trades" :
    rawText.includes("accountant") || rawText.includes("bookkeep") ? "accounting" :
    rawText.includes("marketing") || rawText.includes("agency") ? "marketing agency" :
  ...
  return {
    ...
    websiteContent: (fc.markdown ?? fc.raw_text ?? "").substring(0, 20000),
    grades: {},
  };
}
...
async function runFastIntel(
  ...
  const resolvedBizName = fc?.ogTitle?.split(" | ")?.[0]?.split(" - ")?.[0]?.trim()
    || fc?.title?.split(" | ")?.[0]?.split(" - ")?.[0]?.trim()
  ...
  const bellaOpener = heroQuote
    ? `Hi ${fn}! We've taken a proper look at ${resolvedBizName}, and a few things stood out straight away. ${sitePraise ? sitePraise + ". " : ""}Your hero message — "${heroQuote}" — does a great job of signalling what you do. It looks like you're mainly targeting ${icpGuess}.`
    : `Hi ${fn}! We've taken a proper look at ${resolvedBizName} — great to have you here. ${sitePraise || `Your site is looking sharp`}.`;
  ...
  const fastIntel: FastIntelResult = {
    ...
    core_identity: {
      ...
      industry:      consultant?.industry ?? (fc ? inferIndustry(fc) : ""),
      location:      fc?.address?.replace(/\d{4}.*/, "").trim() || "",
    ...
    flags: {
      ...
      is_running_ads:             (fc as any)?.tech_stack?.is_running_ads   ?? false,
      is_retargeting:             (fc as any)?.tech_stack?.is_retargeting   ?? false,
      has_fb_pixel:               (fc as any)?.tech_stack?.has_fb_pixel     ?? false,
      has_google_ads:             (fc as any)?.tech_stack?.has_google_ads   ?? false,
      has_tiktok_ads:             (fc as any)?.tech_stack?.has_tiktok_ads   ?? false,
      has_multi_platform_ads:     (fc as any)?.tech_stack?.has_multi_platform_ads ?? false,
      // Lead handling
      speed_to_lead_needed:       (fc as any)?.tech_stack?.speed_to_lead_needed ?? !!(fc as any)?.has_chat === false,
      call_handling_needed:       (fc as any)?.tech_stack?.call_handling_risk ?? !!(fc as any)?.phone,
      no_chat:                    (fc as any)?.tech_stack?.no_chat           ?? true,
      no_crm:                     (fc as any)?.tech_stack?.no_crm            ?? true,
      no_booking_tool:            (fc as any)?.tech_stack?.no_booking_tool   ?? true,
      // Database signals — if they have email tool or ecommerce, they have a database
      database_reactivation:      (fc as any)?.tech_stack?.database_likely   ?? false,
...
async function writeFastIntelToKV(
  ...
  const envelope: Record<string, any> = {
    ...
    // Deep intel placeholder — filled by apify-intel workflow
    intel: {
      grade:        "pending",
      bella_opener: fastIntel.bella_opener,
  ...
  await Promise.all([
    ...
    env.LEADS_KV.put(lid, str, { expirationTtl: KV_TTL }),
  ]);

  log("KV", `Written lid=${lid} biz="${fastIntel.core_identity.business_name}" fn="${fn}" ${str.length} bytes`);
}
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (url.pathname === "/status" && request.method === "GET") {
      ...
      const raw = await env.LEADS_KV.get(`lead:${lid}:intel`) ?? await env.LEADS_KV.get(lid);
      if (!raw) {
      ...
      const fastIntelDone = !!data.fast_intel?.status && data.fast_intel.status === "done";
