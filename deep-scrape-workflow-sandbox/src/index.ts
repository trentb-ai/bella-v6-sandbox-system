/**
 * deep-scrape-workflow-sandbox v1.1.0
 *
...
export class DeepScrapeWorkflow extends WorkflowEntrypoint<Env, DeepScrapeParams> {
  async run(event: WorkflowEvent<DeepScrapeParams>, step: WorkflowStep) {
    ...
    console.log(`[DeepScrape v1.1] Starting all actors concurrently lid=${lid}`);
    ...
    await step.do(
      ...
      async () => {
        ...
        await this.env.LEADS_KV.put(lid, enrichedStr, { expirationTtl: KV_TTL });
    ...
    await step.do(
      ...
      async () => {
        ...
        // Raw backup
        await this.env.LEADS_KV.put(`lead:${lid}:deepIntel`, JSON.stringify(deepSummary), { expirationTtl: KV_TTL });
        ...
        const envelope = existing
          ...
          : { v: 1, lid, ts: new Date().toISOString(), intel: {} };
        (envelope.intel as Record<string, unknown>).deep = deepSummary;
        await this.env.LEADS_KV.put(`lead:${lid}:intel`, JSON.stringify(envelope), { expirationTtl: KV_TTL });

        // Mirror into bare lid key (for MCP worker compatibility)
    ...
    console.log(`[DeepScrape v1.1] Complete lid=${lid} wall=${Date.now() - t0}ms`);
  }
}
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === "/trigger" && request.method === "POST") {
      ...
      const instance = await env.DEEP_SCRAPE_WORKFLOW.create({
        id: `deep-${lid}`,  // idempotent — won't double-run same lid
