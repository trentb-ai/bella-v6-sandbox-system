/**
 * bella-voice-agent v5.0.0
 * Architecture: Browser WebSocket → Durable Object → Deepgram Voice Agent WS
 * v5.0.0: Director opening with SC ranking, 5-agent ROI, parallel KV intel (layered on MCP)
 */
...
interface Env {
  ...
  TOOLS: Fetcher;              // V5: bella-tools-worker-v5
  CONSULTANT: Fetcher;         // V5: consultant-sandbox-v5 (freeform Tier 2)
  DEEPGRAM_API_KEY: string;
  MCP: Fetcher;                // leads-mcp-worker-sandbox-v5 (service binding)
...
// v5.0.0: expanded state for SC ranking + ROI data
...
export class BellaAgent extends Agent<Env, BellaState> {
  ...
  private async loadIntelAndConnect(connection: any, t0: number, retries = 0) {
    ...
    // ── v5.0.0: If KV pipeline intel exists, use structured SC prompt ────────
    if (kvIntel) {
      this.systemPrompt = this.buildSystemPromptV3(kvIntel, intel.business_name ?? "your business", basePrompt);
      log("INTEL", `using v5.0.0 structured prompt (${this.systemPrompt.length} chars)`, t0);
...
The opening greeting has just been spoken. Begin Stage 1: ask their role, then what brings them here today.`;
    ...
    // ── v5.0.0: Director opening when SC + ROI data available ────────────────
  ...
  // ── v5.0.0: Structured system prompt from SC intel ─────────────────────────
    ...
    dgWs.addEventListener("open", () => {
      ...
      const settings: Record<string, any> = {
        ...
        agent: {
          ...
          think: {
            // V5: BYO LLM bridge — Gemini Flash 2.0 via deepgram-bridge-v5
            provider: {
              type: "custom",
