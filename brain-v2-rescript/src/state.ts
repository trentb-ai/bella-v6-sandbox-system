...
export async function loadState(storage: DurableObjectStorage): Promise<ConversationState | null> {
  ...
  if (state.confirmedICP === undefined) state.confirmedICP = null;
  if (state.overriddenICP === undefined) state.overriddenICP = null;
  if (state.confirmedCTA === undefined) state.confirmedCTA = null;
  if (state.overriddenCTA === undefined) state.overriddenCTA = null;
  if (!state.lowConfidence) state.lowConfidence = null;
  if (!state.userOverrides) state.userOverrides = null;
