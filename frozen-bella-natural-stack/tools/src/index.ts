 ...
 *
 * Schema: all KV access via kvKey from shared/kv-schema.ts
 */

// Inlined from golden source (shared/kv-schema was a monorepo-only path)
const kvKey = {
...
async function getLeadData(lid: string, env: Env) {
  // Schema-aligned: only read canonical keys (no standalone :name key)
  const [intelJson, bellaJson] = await Promise.all([
    env.LEADS_KV.get(`lead:${lid}:intel`),
    env.LEADS_KV.get(`lead:${lid}:bella:plan`),
  ]);
  if (!intelJson) return null;
  let intel: any = {}, bellaPlan: any = {};
  try { if (intelJson) intel = JSON.parse(intelJson); } catch {}
  try { if (bellaJson) bellaPlan = JSON.parse(bellaJson); } catch {}
  // Name comes from intel envelope, not standalone key
  const firstName = intel.first_name || intel.firstName || intel.contact?.name || "";
  return {
    ...intel,
...
async function handleResolveIntelHot(req: Request, env: Env) {
  ...
  // ROI reads — schema-aligned: single canonical key
  ...
  let roiResults: Record<string, any> = {};
  let totalMonthlyROI = 0;
  const roiStr = await env.LEADS_KV.get(`lead:${lid}:roi`);
  if (roiStr) {
    try {
      const roiData = JSON.parse(roiStr);
      roiResults = roiData.agents ?? roiData.results ?? {};
      totalMonthlyROI = roiData.total_monthly ?? Object.values(roiResults).reduce((sum: number, r: any) => sum + Number(r.monthly_opportunity ?? 0), 0);
    } catch {}
  }
...
async function handleSaveLeadPatch(req: Request, env: Env) {
  ...
  const lid = String(body.lid ?? "").trim();
  if (!lid) return j({ error: "lid required" }, 400);
  // Schema-aligned: read from canonical callBrief key (V8) then patch and write back
  const existing = await env.LEADS_KV.get(kvKey.callBrief(lid));
  let data: any = {};
  try { if (existing) data = JSON.parse(existing); } catch {}
  await env.LEADS_KV.put(
    kvKey.callBrief(lid),
    JSON.stringify({ ...data, ...body.patch, _updated: new Date().toISOString() }),
    { expirationTtl: kvTTL.callBrief }
  );
...
async function handleGetRoiConfirmed(req: Request, env: Env) {
  ...
  // Schema-aligned: read from single canonical ROI key (not per-agent keys)
  ...
  if (roiStr) {
    try {
      const roiData = JSON.parse(roiStr);
      results = roiData.agents ?? roiData.results ?? {};
      totalMonthly = roiData.total_monthly ?? Object.values(results).reduce((sum: number, r: any) => sum + Number(r.monthly_opportunity ?? 0), 0);
    } catch {}
  }
...
async function handleRunDeepAnalysis(req: Request, env: Env) {
  ...
  // Schema-aligned: use conv_memory (canonical) instead of memory
  const [intelStr, memoryStr] = await Promise.all([
    env.LEADS_KV.get(`lead:${lid}:intel`),
    env.LEADS_KV.get(`lead:${lid}:conv_memory`),
  ]);

  let intel: any = {}, memory: any = {};
  try { if (intelStr) intel = JSON.parse(intelStr); } catch {}
  try { if (memoryStr) memory = JSON.parse(memoryStr); } catch {}

  const websiteUrl = intel.websiteUrl ?? intel.core_identity?.website ?? "";
  const businessName = intel.core_identity?.business_name ?? intel.business_name ?? "";
  ...
  async fetch(req: Request, env: Env): Promise<Response> {
    ...
    if (snapMatch) {
      ...
      const [intel, memory] = await Promise.all([
        ...
        env.LEADS_KV.get(`lead:${lid}:conv_memory`, { type: "json" }),
      ]);
    ...
    if (path === "/health") {
      return j({ status: "ok", worker: "bella-tools-worker-v9", version: "6.0.0", endpoints: [
