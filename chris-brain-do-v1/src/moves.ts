...
export function buildStageDirective(input: StageDirectiveInput): StageDirective {
  ...
  switch (stage) {
    ...
    case 'greeting': {
      ...
      const greetBiz = biz(state);
      console.log(`[GREETING_RESOLVE] ts=${new Date().toISOString()} name=${greetName} biz=${greetBiz.slice(0, 30)}`);
      return {
        objective: 'Warm opening — Chris introduces himself as website concierge.',
        allowedMoves: ['advance:discovery'],
        requiredData: [],
        speak: `Hi! I'm Chris, the ${greetBiz || 'website'} AI concierge. I've already been through the site — what can I help you with today?`,
        ask: false,
        waitForUser: true,
        canSkip: false,
        advanceOn: ['user_replied'],
...
James (reputation manager): Automates Google review collection and management.`,
    ...
    case 'discovery': {
      const primaryCTA = (state as any).chrisPrimaryCTA || 'book a consultation';
      return {
        objective: 'Open-ended discovery — understand what the visitor needs.',
        allowedMoves: ['advance:cta_push'],
        requiredData: [],
        speak: '',
        ask: true,
        waitForUser: true,
        canSkip: false,
        notes: [
          'Listen and respond to what the visitor says. Use site intel as background knowledge only.',
          'Do NOT read intel as a list. React naturally.',
          `After 5+ turns or when the visitor shows strong intent, transition toward: "${primaryCTA}".`,
        ],
      };
    }

    case 'cta_push': {
      const cta = (state as any).chrisPrimaryCTA || 'book a consultation';
      return {
        objective: `Drive toward the primary CTA: "${cta}".`,
        allowedMoves: ['advance:handoff'],
        requiredData: [],
        speak: '',
        ask: true,
        waitForUser: true,
        canSkip: false,
        notes: [
          `Naturally steer the conversation toward: "${cta}".`,
          'When the visitor agrees or shows strong intent, confirm and transition to handoff.',
        ],
      };
    }
    ...
    case 'handoff': {
      const cta = (state as any).chrisPrimaryCTA || 'book a consultation';
      return {
        objective: 'Confirm the handoff action and close the conversation.',
        allowedMoves: [],
        requiredData: [],
        speak: `I can get that sorted for you right now — ${cta}.`,
        ask: false,
        waitForUser: true,
        canSkip: false,
        notes: ['Handoff is terminal — do not advance beyond this stage.'],
      };
    }
