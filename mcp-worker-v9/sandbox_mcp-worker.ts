...
async function getLeadData(lid, env2) {
  // Schema-aligned: only read canonical keys (no bare lid, no standalone :name)
  ...
  // Name comes from intel envelope (firstName or first_name field)
  ...
  const merged = {
    ...
    // SC v9 fields — pass through directly
...
function flattenForBella(lid, data) {
  ...
  return {
    ...
    // Routing — SC v9 agent_ranking wins over old routing.top_agents
    ...
    // SC v9 passthrough — Bella uses these for tool calls
    agent_ranking: (() => {
    ...
    // Consultant plan — SC v9 bella_opener is the primary hook
...
async function run(name, args, env2) {
  try {
    ...
    if (name === "write_live_data") {
      // Schema-aligned: outcome writes are delegated to bella-tools only
      // This tool now just logs the intent — actual write happens via bella-tools /write_outcome
      const lid = args.lid;
      if (!lid) return { content: [{ type: "text", text: "Error: lid required" }], isError: true };
      const ts = (/* @__PURE__ */ new Date()).toISOString();
      console.log(JSON.stringify({ event: "write_live_data_delegated", lid, qualified: args.qualified === true, booked: args.booked === true, ts, note: "Call bella-tools /write_outcome instead" }));
      return { content: [{ type: "text", text: "Note: outcome write delegated to bella-tools for lid " + lid }] };
    }
...
var index_default = {
  async fetch(req, env2) {
    ...
    if (path === "/health") return j({ status: "ok", version: "1.6.0", tools: TOOLS.map((t) => t.name), routes: ["/health", "/ghl-contact-created", "/resolve-intel", "/lead", "/status/:id", "/event/:id", "/mcp"] });
    ...
    if (path === "/resolve-intel") {
      ...
      // Schema-aligned: read from single canonical ROI key
      const topAgents = (out.agent_ranking ?? out.top_agents ?? []).slice(0, 5);
      const roiTopFixes = [];
      let totalRoiMonthly = 0;
      const roiStr = await env2.LEADS_KV.get("lead:" + lid + ":roi");
      if (roiStr) {
        try {
          const roiData = JSON.parse(roiStr);
          const agents = roiData.agents ?? roiData.results ?? {};
          for (const agent of topAgents) {
            const roi = agents[agent];
            if (roi) {
    ...
    // ── Workflow proxy: /status/:instanceId and /event/:instanceId ───────────
    if (path.startsWith("/status/")) {
      const instanceId = path.split("/status/")[1];
      if (!instanceId) return j({ error: "Missing instanceId" }, 400);
      try {
        const resp = await env2.SCRAPE_WORKFLOW.fetch(new Request("https://workflow" + path));
